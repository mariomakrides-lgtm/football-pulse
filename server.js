...
async function loadMatches(dateFrom, dateTo) {
  ...
  const jobs = [
    Promise.all(dates.map(apiFootballFixtures)).then(results => ({
      name: "API-Football",
      state: results.some(item => item.state === "online") ? "online" : "not configured",
  ...
  const names = ["API-Football", "football-data.org"];
