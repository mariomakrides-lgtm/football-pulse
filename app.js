...
async function loadScores() {
  ...
  try {
    ...
    $("#matches").innerHTML = matches.map(match => {
      ...
      return `
        <article
          class="match-card team-colour-card"
          ...
          <div class="match-bottom">
            ...
            <span class="open-stats">Stats & scorers →</span>
          </div>
        </article>
      `;
    }).join("");
...
function renderMatchDetails(data) {
  ...
  const stats = (data.statistics || []).map(stat => `
    <div class="stat-row">
      ...
      <div class="stat-bar">
        <span style="width:${statWidth(stat.home, stat.away)}%"></span>
      </div>
    </div>
  `).join("");
  ...
  const goalEvents = (data.events || []).filter(event => {
    const text = `${event.type} ${event.detail}`.toLowerCase();
    return text.includes("goal") && !text.includes("disallowed");
  });
  ...
  const scorers = goalEvents.map(event => `
    <div class="scorer-row">
      ...
      <div>
        ...
        <p>
          ${safe(event.team || "")}
          ${event.assist ? ` · Assist: ${safe(event.assist)}` : ""}
        </p>
      </div>
    </div>
  `).join("");
  ...
  $("#modalContent").innerHTML = `
    <div
      class="scoreboard animated-scoreboard"
      style="
        --modal-home:${safe(homeColours[0])};
        --modal-home-accent:${safe(homeColours[1])};
        --modal-away:${safe(awayColours[0])};
        --modal-away-accent:${safe(awayColours[1])};
      ">
      ...
      <div class="modal-score">
        ...
        <strong>
          ${safe(scoreValue(match.homeScore))}
          <b>–</b>
          ${safe(scoreValue(match.awayScore))}
        </strong>
      </div>
    ...
    <section class="scorers-box">
      <div class="modal-section-title">
        <h3>Goalscorers</h3>
        <span>${goalEvents.length} goal event${goalEvents.length === 1 ? "" : "s"}</span>
      </div>

      ${scorers || `
        <div class="empty">
          No goalscorer data has been supplied yet.
          This can depend on the match and your Live-Score API plan.
        </div>
      `}
    </section>
    ...
    <div class="modal-columns">
      <section class="modal-box">
        <div class="modal-section-title">
          ...
          <span>${safe(data.availability?.statistics || "")}</span>
        </div>

        ${stats || `
          <div class="empty">
            Statistics are not available for this match.
            Some competitions or API plans do not include detailed stats.
          </div>
        `}
      </section>
      ...
      <section class="modal-box">
        <div class="modal-section-title">
          ...
          <span>${safe(data.availability?.events || "")}</span>
        </div>

        ${events || `
          <div class="empty">
            No match events have been supplied yet.
          </div>
        `}
      </section>
    </div>
